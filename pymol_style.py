...
def BallnStick(arg1):
    ...
    cmd.color("gray85", f"elem C and {arg1}")
    cmd.color("gray98", f"elem H and {arg1}")
    cmd.color("slate", f"elem N and {arg1}")
...
def BallnStickThick(arg1, color):
...
def prettyProteinBindingSite(chain, binding_residues=None, ligand_resn=None):
    """
    Example:
        PyMOL> prettyProteinBindingSite chainB, binding, LIG
    """
    cmd.show("cartoon", chain)
    cmd.color("gray80", chain)

    if binding_residues:
        cmd.show("sticks", binding_residues)
        # hide nonpolar hydrogens on residues
        cmd.hide("everything", f"(hydro and {binding_residues} and (elem H and not (neighbor elem N+O)))")

    if ligand_resn:
        # Exclude the ligand based on its residue name
        cmd.color("gray80", f"elem C and {chain} and not resn {ligand_resn}")
    else:
        # If no ligand provided, color normally
        cmd.color("gray80", f"elem C and {chain}")
...
cmd.extend("prettyProteinBindingSite", prettyProteinBindingSite)
cmd.extend("setPretty", setPretty)
cmd.extend("BallnStick", BallnStick)
cmd.extend("BallnStickThick", BallnStickThick)
